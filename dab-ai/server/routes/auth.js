...
function sanitizeUser(user, requestingRole) {
  ...
  return {
    ...
    widgetColor: user.widgetColor || '#6C63FF',
...
router.patch('/settings', requireAuth, async (req, res) => {
  try {
    const updates = {
      ...
      onboardingComplete: req.body?.onboardingComplete != null ? Number(req.body.onboardingComplete) : undefined,
      greetingMessage: asString(req.body?.greetingMessage, 'greetingMessage', { max: 600 }),
      widgetColor: asString(req.body?.widgetColor, 'widgetColor', { max: 30 }),
