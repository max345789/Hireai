...
router.post('/webhook/meta', async (req, res) => {
  ...
  setImmediate(async () => {
    for (const msg of parsed) {
      ...
      if (!matchedUserId) {
        logger.warn('unmatched_meta_webhook', {
          channel: msg.channel,
      ...
      try {
        ...
        let lead = await Lead.findByContact({ phone: pseudoPhone, email: null, channel: msg.channel, userId: matchedUserId });
        ...
        if (!lead) {
          ...
          const newActivity = await ActivityLog.create({
            leadId: lead.id,
            userId: lead.userId || matchedUserId || null,
            leadName: lead.name,
            action: 'replied',
            channel: msg.channel,
        ...
        if (lead && !lead.userId && matchedUserId) {
          lead = await Lead.update(lead.id, { userId: matchedUserId });
        }
