...
function baseUrl(req) {
  return env.baseUrl || requestOrigin(req);
}

function frontendUrl(req) {
  return env.frontendUrl || env.corsOrigins[0] || requestOrigin(req) || null;
}

function buildGoogleConfig(req) {
  const clientId = process.env.GOOGLE_CLIENT_ID;
  const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
  const resolvedBaseUrl = baseUrl(req);
  const redirectUri = resolvedBaseUrl ? `${resolvedBaseUrl}/api/integrations/google/callback` : null;
  return { clientId, clientSecret, redirectUri };
}

function buildMetaOAuthUrl(req, state) {
  const appId = process.env.META_APP_ID;
  const resolvedBaseUrl = baseUrl(req);
  const redirectUri = resolvedBaseUrl ? `${resolvedBaseUrl}/api/integrations/meta/callback` : null;
...
router.post('/integrations/:provider/connect', requireAuth, async (req, res) => {
  ...
  if (provider === 'meta' || provider === 'instagram' || provider === 'messenger') {
    ...
    const url = buildMetaOAuthUrl(req, state);
...
async function handleIntegrationCallback(req, res) {
  ...
  const resolvedFrontendUrl = frontendUrl(req);

  if (!state) {
    if (shouldRedirect && resolvedFrontendUrl) return res.redirect(`${resolvedFrontendUrl}/inbox?integration_error=missing_state`);
  ...
  try {
    ...
    if (shouldRedirect && resolvedFrontendUrl) return res.redirect(`${resolvedFrontendUrl}/inbox?integration_error=invalid_state`);
    return res.status(400).json({ error: 'Invalid state token' });
  }

  const userId = Number(decoded?.userId || 0);
  if (!userId) {
    if (shouldRedirect && resolvedFrontendUrl) return res.redirect(`${resolvedFrontendUrl}/inbox?integration_error=invalid_user`);
    return res.status(400).json({ error: 'Invalid user in state' });
  }

  if (provider === 'google') {
    if (!code) {
      if (shouldRedirect && resolvedFrontendUrl) return res.redirect(`${resolvedFrontendUrl}/inbox?google_error=missing_code`);
    ...
    try {
      ...
      if (shouldRedirect && resolvedFrontendUrl) {
        return res.redirect(`${resolvedFrontendUrl}/inbox?google_connected=1`);
      }

      return res.json({ success: true, provider: 'google' });
    } catch (error) {
      if (shouldRedirect && resolvedFrontendUrl) {
        return res.redirect(`${resolvedFrontendUrl}/inbox?google_error=${encodeURIComponent(error.message)}`);
      }
  ...
  if (provider === 'meta' || provider === 'instagram' || provider === 'messenger') {
    ...
    if (code && process.env.META_APP_ID && process.env.META_APP_SECRET) {
      try {
        const redirectBase = baseUrl(req);
        const redirectUri = redirectBase ? `${redirectBase}/api/integrations/meta/callback` : null;
    ...
    if (shouldRedirect && resolvedFrontendUrl) {
      if (next.accessToken) {
        return res.redirect(`${resolvedFrontendUrl}/inbox?meta_connected=1`);
      }
      return res.redirect(
        `${resolvedFrontendUrl}/inbox?meta_error=${encodeURIComponent(next.oauthError || 'oauth_incomplete')}`
      );
    }
