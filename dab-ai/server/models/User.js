...
class User {
  ...
  static async updateSettings(id, updates) {
    ...
    const next = {
      ...
      selectedAgent: updates.selectedAgent ?? current.selectedAgent,
      accentColor: updates.accentColor ?? current.accentColor,
      slackWebhook: updates.slackWebhook ?? current.slackWebhook,
      notificationPrefs: updates.notificationPrefs ?? current.notificationPrefs,
      widgetId: updates.widgetId ?? current.widgetId,
      widgetSecret: updates.widgetSecret ?? current.widgetSecret,
    ...
    await db.run(
      `UPDATE users
       SET agencyName = ?, twilioKey = ?, gmailConfig = ?, metaConfig = ?,
           ...
           greetingMessage = ?, widgetColor = ?, stripeCustomerId = ?, selectedAgent = ?,
           accentColor = ?, slackWebhook = ?, notificationPrefs = ?, widgetId = ?, widgetSecret = ?, role = ?
       WHERE id = ?`,
      [
        next.agencyName,
        next.twilioKey,
